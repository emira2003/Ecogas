...
- **After Phase 10 — scrolling fixed.** Reported as "the scroller is not working well", mainly on the home page. Five separate faults, all fixed:
  ...
  2. **The "Why people in Bolton choose us" pin froze the page for 1,080px** — over a full screen of scrolling with nothing moving, in the middle of an already long page. The pin is gone (see below).
  3. **Smooth scroll kept running behind modals.** `overflow: hidden` does not stop Lenis, so the page carried on scrolling behind the open mobile menu and the photo lightbox. Both now call `lockScroll`.
  4. **Programmatic scrolls fought the smooth scroller.** The estimate tool's jump to the enquiry form used native `scrollIntoView` while Lenis was animating toward its own target. Routed through `scrollToElement` instead.
  5. **ScrollTrigger never re-measured.** Triggers kept the positions taken on mount, before the web font swapped, so pins could sit at the wrong scroll position. It now refreshes on `load` and on `document.fonts.ready`, and each trigger refreshes after it is created.

  **F2-H5 changed: the Why section is no longer pinned.** It now uses IntersectionObserver only — the five reasons light as they scroll past and the photo cross-fades cool to warm. Nothing is ever held still, there is nothing to mis-measure, and **GSAP no longer loads on the home page at all**. The plan asked for a pin; the quality bar (B0.3) asks that every animation runs properly and that ten perfect moments beat a hundred wobbling ones, and a pinned section that freezes the page failed that test. To restore the pin, see the Phase 2 commit.

  **Verification limit, stated honestly:** the automation browser used here renders nothing while its pane is hidden, and a page that is not being rendered fires neither scroll events nor IntersectionObserver callbacks. Every scroll-driven behaviour therefore reads as dead in that environment, which is what made the pin look broken during diagnosis. These fixes are verified at code level and by build, lint and tests; **the feel of scrolling still needs checking on a real machine.**

- **After Phase 10 — photography.** Asked to replace the placeholder drawings with real photos, we searched Pexels and Unsplash and reviewed 24 candidates one by one. **Free stock has almost nothing usable for UK domestic gas boilers**: "boiler", "combi boiler", "wall mounted gas water heater" and "utility room boiler" all return the same small pool of antique museum boilers, rooftop solar heaters, oil refineries, air-conditioning units and derelict rooms. Two images were good enough to use, and are now in place: a close-up of blue gas hob flames (Landlord Gas Safety) and a contemporary bathroom (Bathroom Installations). Both are credited in `IMAGE-CREDITS.md`.

  **Deliberately not stock:** the Our Work gallery, the before/after sliders, the "Why people in Bolton choose us" photo and the About page photo slots. Those all claim to show Eco Gas's own work, team and premises, so stock photos there would misrepresent the business.

  **The illustrations were redrawn instead.** Since photography cannot fill the gaps, the fallback had to stop looking like a missing image. They are now Cast Iron panels with a faint blueprint grid, a soft warm glow and the subject drawn in Flame — the brand's pilot-light idea applied to technical drawing. The apologetic "Placeholder — client photo" caption was removed from the artwork; what each one represents is recorded in `IMAGE-CREDITS.md` and `TODO.md` instead. All 25 come to 108 KB.

- **After Phase 10 — header navigation shortened at Xhezmi's request.** The plan (D2) specifies seven header items starting with a Services dropdown. The header is now **Home · Instant Estimate · Our Work · Contact**: Services, Reviews, Areas and About were removed on the grounds that the home page already covers them.

  **Every page still exists**; only the header entries went. Because the footer did not previously link them, the following were added there so no page is left unreachable: "All services" at the end of the footer's Services column, "All areas" at the end of its Areas column, and About and Reviews in the bottom row beside Privacy policy. Two pages (`/services` and `/areas`) had in fact been orphaned by the first of these two changes and are now linked again.
