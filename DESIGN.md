...
- **After Phase 10 — header navigation changed at Xhezmi's request.** The plan (D2) specifies a Services dropdown as the first header item. That has been replaced by a Home link, on the grounds that the home page already lists the services. The seven service pages are still linked from the home page tiles, from the footer on every page, from each service page's "Related services", and from the area pages. The trade-off noted at the time: the header dropdown was the strongest internal link to the pages that carry the local search terms, so if rankings for "boiler replacement Bolton" and similar matter more than a tidy header, it is worth revisiting. Restoring it means putting the Services entry back in `src/components/layout/nav.ts` and re-adding the dropdown markup to `Header.tsx` (see the Phase 1 commit); the `.nav-dropdown` and `.mobile-nav__sublink` styles were left in `globals.css` ready for it.
